        ...
        <div className="min-h-[calc(100vh-80px)] bg-slate-50 py-12 px-6 sm:px-12 flex items-center justify-center">
            <div className="max-w-3xl w-full bg-white rounded-[3rem] shadow-2xl shadow-slate-200 border border-slate-100 p-8 sm:p-14 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="mb-12">
                        <h2 className="text-3xl font-black text-slate-800 italic tracking-tight">Become a Hero</h2>
                        <p className="text-slate-500 font-medium mt-1">Complete the 5-step registration to join the network.</p>
                    </div>
                    ...
                    <div className="mt-12 min-h-[400px]">
                        ...
                        {currentStep === 3 && (
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 max-h-[450px] overflow-y-auto pr-4 scrollbar-hide">
                                <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl flex items-center justify-between mb-4 sticky top-0 bg-white z-10 border-b shadow-sm">
                                    <div className="flex items-center space-x-2">
                                        <ShieldCheck className="text-emerald-600" size={20} />
                                        <span className="font-bold text-emerald-800 italic">Live Eligibility Check</span>
                                    </div>
                                    <span className={`px-4 py-1 rounded-full text-xs font-black uppercase ${calculateEligibility() ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'}`}>
                                        {calculateEligibility() ? 'Eligible' : 'Check Details'}
                                    </span>
                                </div>
                                    ...
                                    {[
                                        ...
                                        <div key={item.key} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                            <div className="flex-1 mr-4">
                                                <p className="font-bold text-slate-800 text-sm italic">{item.q}</p>
                                                <p className="text-[10px] text-slate-400 uppercase font-medium mt-1">{item.hint}</p>
                                            </div>
                                            <div className="flex bg-white rounded-xl p-1 shadow-sm border border-slate-100">
                                                <button
                                                    onClick={() => setFormData({ ...formData, [item.key]: true })}
                                                    className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${formData[item.key] === true ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
                                                >Yes</button>
                                                <button
                                                    onClick={() => setFormData({ ...formData, [item.key]: false })}
                                                    className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${formData[item.key] === false ? 'bg-primary text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
                                                >No</button>
                                            </div>
                                        </div>
                                    ))}
                    ...
                    <div className="mt-12 flex items-center justify-between">
                        <button
                            ...
                            className={`text-sm font-black text-slate-400 hover:text-primary transition-all flex items-center uppercase tracking-widest ${currentStep === 0 ? 'opacity-0' : ''}`}
                        >
