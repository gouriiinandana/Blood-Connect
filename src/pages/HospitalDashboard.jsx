                    ...
                    >
                        <div className="flex items-center justify-between flex-wrap gap-4">
                            <div className="flex items-center space-x-4">
                                <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center font-bold text-lg shadow-sm ${status === 'critical' ? 'bg-primary text-white border-white/20' : 'bg-slate-100 text-slate-700 border-slate-200'}`}>
                                    {type}
                                </div>
                                <div>
                                    <p className="font-bold text-slate-800">{units} <span className="font-medium text-slate-400 text-sm">units</span></p>
                                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] border ${cfg.color}`}>
                            ...
                            <div className="flex items-center space-x-3">
                                <div className="flex items-center space-x-1 bg-slate-50 p-1 rounded-xl border border-slate-100">
                                    <button
                                        ...
                                        disabled={isSaving}
                                        className="w-10 h-10 rounded-xl bg-white font-bold text-slate-700 hover:bg-red-50 hover:text-red-600 transition-all text-lg shadow-sm disabled:opacity-40 border border-slate-100"
                                    >−</button>
                                    ...
                                    <button
                                        onClick={() => handleUnitChange(type, +5)}
                                        disabled={isSaving}
                                        className="w-10 h-10 rounded-xl bg-white font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-600 transition-all text-lg shadow-sm disabled:opacity-40 border border-slate-100"
                                    >+</button>
                                </div>
                                <span className={`text-[10px] font-bold uppercase tracking-[0.1em] px-4 py-2 rounded-xl border ${cfg.color}`}>{cfg.label}</span>
                            </div>
                        </div>
                    </div>
                );
        ...
        <div className="space-y-8 pb-12">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <h1 className="text-4xl font-bold text-slate-900 tracking-tight">Hospital Command Center</h1>
            ...
            {activeEmergencies.length > 0 && (
                <div className="bg-red-50 border-2 border-red-200 rounded-3xl p-6 space-y-4">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <p className="text-[10px] font-bold text-red-500 uppercase tracking-[0.2em]">Active Broadcasts</p>
                            <p className="font-bold text-red-700">{activeEmergencies.length} EMERGENCY ALERT{activeEmergencies.length > 1 ? 'S' : ''}</p>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {activeEmergencies.map(em => (
                            <div key={em.id} className="bg-primary text-white p-4 rounded-2xl text-center shadow-lg shadow-primary/25">
                                <p className="text-3xl font-bold">{em.type}</p>
                                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/70 mt-2">Active Since {em.time}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    ...
                    <Card key={i} className="group hover:border-secondary/30 transition-all cursor-pointer">
                        ...
                        <p className="mt-4 text-4xl font-bold text-slate-800 tracking-tight">{stat.value}</p>
                        ...
                        <div className="flex items-center mt-3 text-emerald-600">
                            ...
                            <p className="text-xs font-bold uppercase tracking-widest">{stat.change}</p>
                        </div>
                    </Card>
                ))}
            </div>
                    ...
                    <div className="flex space-x-3 mb-8">
                        {['inventory', 'chart'].map(tab => (
                            <button
                                ...
                                className={`px-6 py-3 rounded-2xl text-[10px] font-bold uppercase tracking-[0.2em] transition-all ${activeTab === tab ? 'bg-secondary text-white shadow-xl shadow-secondary/20' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}
                            >
                                ...
                                <div key={req.id} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-primary/20 transition-all">
                                    <div className="flex items-center space-x-4">
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg shadow-sm
                                            ${req.urgency === 'critical' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-slate-100 text-slate-600'}`}>
                                            {req.bloodType}
                                        </div>
                                        <div>
                                            <p className="font-bold text-slate-800">{req.hospital}</p>
                                            <p className="text-xs text-slate-400 font-bold uppercase tracking-[0.1em]">{req.units} units · {req.distance} away</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center space-x-3">
