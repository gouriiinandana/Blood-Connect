...
const ProcessStep = ({ number, title, desc, delay }) => {
    ...
    return (
        ...
        >
            <div className="shrink-0 w-12 h-12 rounded-2xl bg-primary text-white flex items-center justify-center font-black text-lg shadow-lg shadow-primary/20 group-hover:scale-110 transition-transform">
                {number}
            </div>
            <div className="pt-1.5">
                <p className="font-black text-slate-800 text-base leading-tight">{title}</p>
                {desc && <p className="text-slate-500 text-sm mt-0.5 font-medium">{desc}</p>}
            </div>
        </div>
    );
};
            ...
            <div ref={contentRef} className="max-w-4xl mx-auto px-6 py-20 space-y-24">
                ...
                <Section>
                    <div className="flex items-center space-x-3 mb-10">
                        ...
                        <div>
                            <p className="text-[10px] font-black text-primary uppercase tracking-widest">Step by Step</p>
                            <h2 className="text-3xl font-black text-slate-800 tracking-tight">How the Donation Process Works</h2>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-50 rounded-3xl p-8 border border-slate-100 space-y-6">
                        ...
                        {/* Total time badge */}
                        <div className="mt-4 pt-6 border-t border-slate-200 flex items-center space-x-3">
                            <Clock size={18} className="text-secondary" />
                            <p className="text-slate-700 font-black text-sm">
                                Total time: <span className="text-secondary">30–45 minutes</span>
                            </p>
                        </div>
                    </div>
                </Section>
                ...
                <Section delay={100}>
                    <div className="flex items-center space-x-3 mb-10">
                        ...
                        <div>
                            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">100% Verified</p>
                            <h2 className="text-3xl font-black text-slate-800 tracking-tight">Is Blood Donation Safe?</h2>
                        </div>
                    </div>
                ...
                <Section delay={100}>
                    <div className="flex items-center space-x-3 mb-10">
                        <div className="w-10 h-10 bg-amber-50 rounded-2xl flex items-center justify-center">
                            <Apple className="text-amber-500" size={22} />
                        </div>
                        <div>
                            <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest">Donor Care</p>
                            <h2 className="text-3xl font-black text-slate-800 tracking-tight">Before & After Donation</h2>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                        ...
                        <div className="bg-blue-50 border border-blue-100 rounded-3xl p-7">
                            <p className="text-xs font-black text-secondary uppercase tracking-widest mb-5">Before Donation</p>
                            <ul className="space-y-4">
                                {['Eat a healthy meal', 'Drink plenty of water', 'Avoid alcohol & smoking', 'Get good sleep'].map((item, i) => (
                                    <Section key={i} delay={i * 80}>
                                        <li className="flex items-center space-x-3">
                                            <span className="w-6 h-6 bg-secondary text-white rounded-lg flex items-center justify-center text-[10px] font-black shrink-0">{i + 1}</span>
                        ...
                        <div className="bg-primary/5 border border-primary/10 rounded-3xl p-7">
                            <p className="text-xs font-black text-primary uppercase tracking-widest mb-5">After Donation</p>
                            <ul className="space-y-4">
                                {['Rest for 10–15 minutes', 'Drink plenty of fluids', 'Avoid heavy exercise for 24 hours', 'Keep the bandage for a few hours'].map((item, i) => (
                                    <Section key={i} delay={i * 80}>
                                        <li className="flex items-center space-x-3">
                                            <span className="w-6 h-6 bg-primary text-white rounded-lg flex items-center justify-center text-[10px] font-black shrink-0">{i + 1}</span>
                                            <span className="text-slate-700 font-semibold text-sm">{item}</span>
                                        </li>
                                    </Section>
                                ))}
                ...
                <Section delay={100}>
                    <div className="flex items-center space-x-3 mb-10">
                        ...
                        <div>
                            <p className="text-[10px] font-black text-primary uppercase tracking-widest">Our Mission</p>
                            <h2 className="text-3xl font-black text-slate-800 tracking-tight">How Blood Connect Helps</h2>
                        </div>
                    </div>
                ...
                <Section delay={100}>
                    <div className="relative bg-gradient-to-br from-primary to-red-800 rounded-[2.5rem] p-12 text-white text-center overflow-hidden">
                        ...
                        <div className="relative">
                            ...
                            <h2 className="text-4xl sm:text-5xl font-black tracking-tight mb-4">Be a Lifesaver Today</h2>
                            <p className="text-red-100 text-lg font-medium max-w-lg mx-auto mb-2">
                                One small act can make a big difference.
                            </p>
                            <p className="text-red-200 text-sm font-medium mb-10">
                                Join Blood Connect and help ensure no life is lost due to lack of blood.
                            </p>
                            <div className="flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4">
                                <Link to="/register">
                                    <button className="flex items-center space-x-2 bg-white text-primary font-black px-8 py-4 rounded-2xl hover:bg-red-50 transition-all shadow-xl hover:scale-105 text-sm uppercase tracking-widest">
                                        <Heart size={18} />
                                        <span>Register as Donor</span>
                                    </button>
                                </Link>
                                <Link to="/hospital/login">
                                    <button className="flex items-center space-x-2 bg-white/10 backdrop-blur text-white border border-white/20 font-black px-8 py-4 rounded-2xl hover:bg-white/20 transition-all text-sm uppercase tracking-widest">
                                        <ArrowRight size={18} />
